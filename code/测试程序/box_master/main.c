            ...
            if(page_n.ts_page_cnt > 4000)
            {
                ...
                if(Drawer_cont_ls(&rb_Flag,R_TsBuffer,R_RsBuffer,TsBuffer,
                                  RsBuffer,Drawer_back,&Rc_ok_Flag) == 0) {
                                     Display_LCD_String_XY(1,1,"回来好的---:",1);
                                  } else {
                                        Display_LCD_String_XY(1,1,"回来错误---:",1);
                                  }
//                if(page_n.page < 4)
//                {
//                    page_n.page++;
...
//                Menu_Host(2,addre,RsBuffer,Dr_Num_Save,&page_n);
        ...
        else
        {
            if(page_n.ts_up_cnt > 4000)
            {
                ...
                rb_Flag = 1;/*初始化状态标志位*/
                if(Drawer_cont_ls(&rb_Flag,R_TsBuffer,R_RsBuffer,TsBuffer,
                                  RsBuffer,Drawer_out,&Rc_ok_Flag) == 0) {
                                     Display_LCD_String_XY(1,1,"  出去好的---:",1);
                                  } else {
                                        Display_LCD_String_XY(1,1,"出去错误---:",1);
                                  }
//                if(page_n.ins_row == 0)
//                {
//                    if(page_n.page == 1)
...
//                Menu_Host(2,addre,RsBuffer,Dr_Num_Save,&page_n);
        ...
        else
        {
            if(page_n.ts_next_cnt > 4000)
            {
                ...
                if(led_ask == 10) {
                    Display_LCD_String_XY(1,1,"开灯坏的---:",1);
                } else {
                    Display_LCD_String_XY(1,1,"开灯好的---:",1);
                }
//                if(page_n.ins_row == 0)
//                {
//                    if(page_n.page == 1)
...
//                        kz_num = RsBuffer[1];
//                        UART_interrupt(0,1);/*使能串口1接收中断*/
//                        UART1_Send(R_TsBuffer,Drawer_out_place,kz_num);
//                        rb_Flag = 5;//重发次数
//                        while(rb_Flag > 0){
//                            WDT();//清看门狗
//                            if(Rc_ok_Flag == 1) {
...
//                                Rc_ok_Flag = 0;
//                                UART_interrupt(0,0);/*关闭串口1接收中断*/
//                                if(R_TsBuffer[1] == 0x01) {//在位置
//                                    jg = ok;
//                                } else {
//                                    jg = no;
//                                }
//                                UART3_Send(TsBuffer,Drawer_out_place,jg);
//                                break;
//                            } else {
//                                static u16 cf_count = 0;
//                                if(cf_count < 45000) {//重发时间
//                                    cf_count++;
//                                } else {
//                                    cf_count = 0;
//                                    UART1_Send(R_TsBuffer,Drawer_out_place,kz_num);
...
//                    if(rb_Flag == 0) {
//                        UART_interrupt(0,0);/*关闭串口1接收中断*/
//                        UART3_Send(TsBuffer,Drawer_out_place,no);
//                    }
                    break;
                    /*机械手回来是否到位*/
                    case Drawer_back_place:
                    cheak_wait_2 = 40;//快速检测
                    Drawer_cont(&rb_Flag,R_TsBuffer,R_RsBuffer,TsBuffer,RsBuffer,Drawer_back_place,&Rc_ok_Flag);
                    break;
                    /*询问机械手使用次数*/
                    case ask_count:
